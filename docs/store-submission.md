...
截至 2026 年 7 月 11 日，三个商店的开发者账号均已注册并可用，Chrome Web Store、Firefox AMO 与 Microsoft Edge Add-ons 均已提交 `0.2.0` 并等待审核。账号就绪不代表扩展已上传、通过审核或发布。仓库准备默认只完成包、文案、隐私政策、审核说明、素材和 go / no-go 证据；真实上传、提交审核、发布或分发范围选择是单独的外部操作，需要用户明确执行或授权。
...
3. 从 `STORE_LISTING.md` 填写英语和简体中文 listing；公开描述不得引用其他浏览器。
...
6. 分类选择当前最接近时间追踪与生产力工作流的 `工作流程与规划`（`Workflow & Planning`）。
7. 首次提交选择 Free、Public、all regions；本次提交启用了审核通过后自动发布。
8. 如果 validator 提到权限、隐私、远程代码或 package 根目录，停止提交并修复，不接受带警告发布。
