...
`url` 是浏览器提供的完整页面 URL，包括 path、query 和 fragment。具名记录在本机保存该值，供数据导出的 `url` / “URL 地址”字段使用，同时从中提取域名用于分类和统计；匿名路径不保存该值。查询参数可能包含搜索词或其他敏感内容，因此商店声明和隐私政策必须明确披露完整 URL。
...
网页记录必须依附当前有效、来源匹配且未暂停的原生活动；连接成功本身不授予计时资格。普通网页依附具名浏览器会话；浏览器或域名选择匿名统计时，接收端在写入前转为无身份活动事实，网页时间只是其中的子集，不重复增加桌面总时长。空闲、暂停、电源和原生截止回溯同样约束匿名区间。
...
匿名规则由桌面端持有，扩展继续发送同一 v1 观察，无需新增匿名标识、下发规则或恢复 tab/window ID。匿名接收成功沿用原响应；扩展不得因响应创建页面历史缓存。URL、标题和图标仅在本机判断中短暂使用，匿名事实不保存这些字段、浏览器客户端标识或具名会话外键。

仅在存在匿名网页规则且当前网页身份无法确认时，桌面端不保存浏览器窗口标题，仍记录有效活动时间。确认普通网页后按原标题偏好处理；确认状态绑定当前窗口、进程、完整标题、规则版本和观察有效期。切换规则、暂停、桥接配置变化或观察失效使确认失效。v1 的同 exe 窗口与配置限制仍按活动资格契约处理。

## 无页面信息的停止通知

自动、周期和手动同步均只报告聚焦窗口中的活动普通标签，发送前重新确认；旧观察不得覆盖新页面。v1 无法让桌面端独立区分同 exe 的窗口或配置，扩展负责确认焦点；缺少可选技术字段时，不得把默认值 `chrome` 当作来源证明。

此前可能已发送网页时，浏览器失焦、标签关闭或切到内部/私密页面须发送 v1 停止通知：`url: "about:blank"`、`incognito: false`，不携带页面标题、图标、实际 URL 或停止原因。技术字段遵循上述同意规则。通知不创建网页记录：具名路径结束同一来源、同一原生会话内尚有效的网页；匿名路径在当前有效原生活动资格内封口匿名区间并撤销网页确认。worker 重启不能丢失尚需停止的状态。
